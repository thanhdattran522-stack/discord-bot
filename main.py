...
async def blacklist_remove(ctx, group_id: int):
    if group_id in DANH_SACH_THEM:
        ...
        save_dynamic_data()
        await ctx.send(f"🗑️ Đã xoá ID `{group_id}`.")
    else: await ctx.send("⚠️ Không thể xoá ID này.")

# --- 3. LỆNH KIỂM TRA HỒ SƠ & CẢNH BÁO ---
@bot.command()
async def kiemtra(ctx, username: str):
    try:
        ...
        # 🎖️ THIẾT LẬP 4 TẦNG LỌC CẢNH BÁO
        warns = []
        if sc_bool: warns.append("🔴 Safe Chat: **BẬT** (Không đạt chuẩn)")
        ...
        # 🖼️ XÂY DỰNG EMBED PROFILE (FIX LỖI NỐI CHUỖI)
        ...
        # Phần 2: Cảnh báo tiêu chuẩn
        if warns:
            desc += "⚠️ **CẢNH BÁO TIÊU CHUẨN:**\n" + "\n".join(warns) + "\n\n"

        # Phần 3: Danh sách đen
    ...
    except Exception as e: await ctx.send(f"⚠️ Lỗi hệ thống: {e}")

# --- 6. LỆNH XEM DANH SÁCH ĐEN ---
@bot.command()
async def check_blacklist(ctx):
    tong_den = list(set(DANH_SACH_DEN_GOC + DANH_SACH_THEM))
    await ctx.send("📡 **Đang trinh sát toàn bộ danh sách đen...**")
    lines = []
