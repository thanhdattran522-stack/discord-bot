...
# Tên kênh phải khớp 100% với tên trên Discord để bot tự dò tìm
...
# DANH SÁCH ĐEN GROUP ID (Cố định)
...
async def kiemtra(ctx, username: str):
    try:
        ...
        # --- BƯỚC 4: XỬ LÝ SAFE CHAT VÀ BLACKLIST ---
        safe_chat_status = "🔴 Bật (Loại)" if info.get("isVieweeSafeChat") else "🟢 Tắt (Bình thường)"
        is_blacklisted = actual_username.lower() in names_in_channel

        # --- BƯỚC 5: TẠO HỒ SƠ QUÂN NHÂN ---
        ...
        embed.add_field(name="🛡️ Safe Chat", value=safe_chat_status, inline=True)
        ...
        # Cảnh báo Blacklist NGUY HIỂM
        alerts = []
        if is_blacklisted: 
            alerts.append(f"💀 **ĐỐI TƯỢNG CÓ TRONG KÊNH #UNIT_BLACKLIST!**")
        if bad_groups: 
